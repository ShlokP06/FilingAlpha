...
from pipeline.ingest import ingest_universe
from pipeline.model import run_walkforward
from pipeline.returns import compute_forward_returns
from scripts.seed_universe import DEFAULT_TICKERS
...
def _print_summary(results: list) -> None:
    ...
    print(f"\n{'signal':<20}{'horizon':>8}{'IC':>9}{'t':>7}{'L-S Sharpe':>12}{'hit':>7}")
    print("-" * 63)
    for r in sorted(results, key=lambda x: abs(x.ic_tstat or 0), reverse=True):
        print(
            f"{r.signal:<20}{r.horizon_days:>8}{(r.ic or 0):>9.3f}{(r.ic_tstat or 0):>7.2f}"
            f"{(r.ls_sharpe or 0):>12.2f}{(r.hit_rate or 0):>7.2f}"
        )
